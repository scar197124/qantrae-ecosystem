...
(function(){
  ...
  function preprocessRawInput(raw){
    ...
    if (markdownMatch) {
      ...
      notes.push("Markdown link wrapper removed; QANTRAE analyzed the destination inside parentheses.");
    ...
    if (htmlAnchorMatch) {
      ...
      notes.push("HTML anchor wrapper removed; QANTRAE analyzed the href destination instead of the visible link text.");
    ...
    if (qrStructuredPrefixes.some(prefix => lowerBoundary.startsWith(prefix))) {
      notes.push("Structured QR/message scheme preserved as payload; QANTRAE did not extract a bare domain from inside it.");
      return { original, value, notes, candidates, markdownLabel };
    }
    ...
    if (schemePrefix && knownNonHttpPayloadSchemes.has(schemePrefix) && !/^https?:\/\//i.test(value)) {
      notes.push("Non-HTTP scheme preserved as payload; QANTRAE did not extract a bare domain from inside it.");
      return { original, value, notes, candidates, markdownLabel };
    }
    ...
    if (!simpleUrlish && explicitPayloadContext && candidates.length >= 1) {
      ...
      notes.push("Multiple URLs were found inside one payload; QANTRAE kept it in payload mode instead of guessing one destination.");
    }
    return { original, value, notes, candidates, markdownLabel };
  }
  ...
  function classifyRedirectIntent(target, messageContext, redirectRoots, queryFindings){
    ...
    if (wrapper) {
      return {active:true, severity:"medium", title:"Redirect intent classifier: wrapper boundary", detail:"v2.67 classified this as a relay, shortener, or tracking wrapper. Offline QANTRAE does not expand it automatically, so the final destination remains unverified.", weight:6, mode:"wrapper"};
    }
  ...
  function makeWhatToDoNextGuidance(risk, target, dominant, calmTriage, evidenceProfile){
    ...
    if (risk === "High Risk") {
      return {
        ...
        whyText:"QANTRAE selected this lane because the strongest local evidence points to " + lane + ". The visible root was " + root + ", but the safest move is to verify outside the original message.",
  ...
  function makeReportConsistencyPack(risk, target, dominant, guidance, evidenceProfile){
    ...
    const eventLine = dominant
      ? "QANTRAE IT matched the " + lane.label + " lane from the strongest local evidence in this " + origin + "."
      : "QANTRAE IT did not find a dominant scam lane in this " + origin + ".";
  ...
  function classifyQrPayload(raw){
    ...
    if (/^(ftp|sftp|chrome-extension|blob|about|content):/i.test(value)) return {kind:"Non-web scheme payload", safety:54, severity:"medium", detail:"This begins with a non-HTTP scheme. QANTRAE kept it out of normal web trust scoring because browsers and apps may handle it differently than a normal website.", workflowHits};
  ...
  function analyze(raw){
    ...
    if (target.type === "dangerous-scheme") {
      ...
      timeline.push("QANTRAE blocked normal trust relief because the input uses a script/local content scheme.");
    ...
    if (target.scheme === "unreadable" && /^https?:\/\//i.test(target.raw)) {
      ...
      timeline.push("URL parser failed; QANTRAE escalated instead of treating it as harmless text.");
      return finalize({target, score, signals, technical, trust, timeline});
    }
    ...
    if (target.type === "payload") {
      ...
      if (recoveredEncodedUrls.length) {
        ...
        addSignal(signals, "high", "Percent-encoded URL recovered", "The message contained a URL hidden behind percent encoding. QANTRAE decoded the candidate locally and deep-scanned the recovered destination.", 26);
        technical.push("v3.38 encoded-message recovery: recovered " + recoveredEncodedUrls[0] + " from percent-encoded text.");
        timeline.push("QANTRAE decoded a URL-shaped percent-encoded message before classification.");
      ...
      if (payload.urls && payload.urls.length) {
        addSignal(signals, "medium", "Embedded link present", "Embedded URL sample: " + payload.urls[0] + ". QANTRAE v2.60 deep-scans the embedded destination instead of stopping at payload classification.", 12);
      ...
      timeline.push("QANTRAE separated QR/text payloads from normal URL risk scoring and classified the payload type first.");
      return finalize({target, score, signals, technical, trust, timeline});
    }
    ...
    if (htmlArtifacts.isHtml) {
      ...
      if (htmlArtifacts.scriptRedirects.length) addSignal(signals,"high","JavaScript redirect snippet","The copied content includes script-style redirection. Offline QANTRAE treats copied redirect code as a strong warning sign.",22);
    ...
    if (messageContext.candidates.length > 1) {
      score -= 12;
      addSignal(signals,"medium","Multiple links in one message","The pasted message contains " + messageContext.candidates.length + " link-like targets. QANTRAE scans the active destination but recommends reviewing every link in the original message before clicking.",12);
    }
    ...
    if (credentialIntent.active && !credentialIntent.rootedTrusted) {
      ...
      addSignal(signals,"low","Credential workflow on known root","Login/account/security wording appears on a known local trusted root. QANTRAE records the workflow context without treating it as impersonation by itself.",-2);
    ...
    if (target.markdownLabel) {
      ...
      if (mismatchedHints.length) {
        ...
        timeline.push("Visible-link text was compared against the real destination; QANTRAE found a brand mismatch.");
      } else if (visibleHints.length) {
        addSignal(signals,"low","Visible label matches known family","The visible markdown label references a known family and the destination root appears consistent with that family. QANTRAE still judges the real destination first.",-2);
    ...
    if (target.normalizationNotes && target.normalizationNotes.some(note => /Protocol-relative/i.test(note))) {
      addSignal(signals,"medium","Protocol-relative link","The input began with // instead of a full scheme. QANTRAE normalized it before scoring because this style can hide the destination when copied out of context.",8);
    ...
    if (isPrivateOrLocalHost(host)) {
      ...
      timeline.push("Private/local destination detected; QANTRAE treated it as high-risk outside a known safe context.");
    }
    ...
    if (hasMalformedDomainPunctuation(target)) {
      ...
      timeline.push("Malformed host punctuation found; QANTRAE moved the result into review instead of treating it as clean.");
    } else {
    ...
    if (/(redirect|return|continue|url|target|next)=https?%3a%2f%2f/i.test(target.raw) || /(redirect|return|continue|url|target|next)=https?:\/\//i.test(target.raw)) {
      ...
      if (sameRootRedirect && (isTrusted || bankFamily)) {
        addSignal(signals,"low","Same-root redirect on known root","A redirect-style parameter points back to the same trusted root. QANTRAE notes it, but v2.46 calibration avoids over-punishing normal first-party navigation.",-2);
    ...
    if (queryFindings.redirect.length) {
      const sameRootRedirect = redirectRootsEarly.length && redirectRootsEarly.every(item => item.root === root);
      if (sameRootRedirect && (isTrusted || bankFamily)) {
        addSignal(signals,"low","First-party redirect parameter","Redirect-style parameter found, but it points back to the same known root. QANTRAE notes it without treating it like a cross-site handoff.",-2);
        ...
        timeline.push("Redirect-style query parameter found; QANTRAE marked it for verification before trust.");
      }
    }
    ...
    if (hasEncodedAtOrSlashTrick(target.raw)) {
      ...
      if (sameRootRedirect && (isTrusted || bankFamily) && !/%40|%5c|\\/i.test(target.raw)) {
        addSignal(signals,"low","Encoded same-root redirect separators","Encoded slashes were found inside a same-root redirect parameter on a known root. QANTRAE records the encoding without escalating it as deception.",-1);
    ...
    if (findDoubleExtension(path)) {
      ...
      timeline.push("Double-extension pattern found; QANTRAE escalated the download risk.");
    }
    ...
    if (shortenerRoot) {
      ...
      addSignal(signals, sensitiveContext ? "high" : "medium", sensitiveContext ? "Brand message uses shortened link" : "Shortener detected", sensitiveContext ? "The message uses trusted-brand or pressure wording but sends the user through a shortened link. Offline QANTRAE cannot see the final destination without permission, so treat sensitive actions as unsafe." : "Short links hide the final destination. Offline QANTRAE can warn about the concealment, but should ask permission before any future online expansion.", sensitiveContext ? 28 : 18);
    ...
    if (/%[0-9a-f]{2}/i.test(target.raw) || /%[0-9a-f]{2}/i.test(path)) {
      ...
      if (sameRootRedirect && (isTrusted || bankFamily) && !/%40|%5c|%00|%0d|%0a|%09/i.test(target.raw)) {
        addSignal(signals,"low","Encoded first-party URL value","Encoded characters appear in a same-root parameter on a known root. QANTRAE records this as normal URL encoding unless stronger deception appears.",-1);
      } else {
        score -= 12;
    ...
    if (randomizedRoot && !(isTrusted || bankFamily)) {
      ...
      timeline.push("Randomized-root pattern checked; QANTRAE treated it as supporting evidence rather than an automatic block.");
    }
    ...
    if (pathEmbeds.length && !(isTrusted || bankFamily)) {
      ...
      timeline.push("Trusted-domain-in-path deception found; QANTRAE emphasized the real root over the visible path text.");
    }
    ...
    if (stuffedSubdomains.length && !(isTrusted || bankFamily)) {
      ...
      timeline.push("Trusted-root-stuffed subdomain found; QANTRAE separated the fake front from the real root.");
    }
    ...
    if (lookalikeSecurityWords.length) {
      ...
      timeline.push("Lookalike security-word pattern found; QANTRAE treated it as caution evidence, not an automatic high-risk verdict.");
    }
    ...
    if (lookalikeInstitutionDomains.length && !bankFamily) {
      ...
      timeline.push("Lookalike institution-domain pattern found; QANTRAE treated it as stronger evidence than a generic suspicious word.");
    }
    ...
    if (nearMissInstitutionRoots.length) {
      ...
      timeline.push("Near-miss institution spelling found; QANTRAE checked whether security/account wording made the pattern more serious.");
    }
    ...
    if (shortInstitutionPressureDomains.length) {
      ...
      timeline.push("Short institution alias plus security wording found; QANTRAE treated it as institution-impersonation evidence, while keeping boundary controls for ordinary words.");
    }
    ...
    if (downloadTrap.paramDownloadWords.length && !(downloadTrap.paramExecutable.length || downloadTrap.paramArchive.length)) {
      ...
      if (benignTrustedDownloadParam) {
        addSignal(signals,"low","Trusted download parameter", "A download-style parameter appears on a known root. QANTRAE records it as file-delivery context without escalating unless executable, archive, pressure, or mismatch evidence appears.",-1);
  ...
  function makeLinkAnatomy(target){
    const empty = { status:"Waiting for input", protocol:"—", host:"—", root:"—", path:"—", query:"—", tokens:"—", note:"QANTRAE will break the target into readable parts after a scan." };
    ...
    if (target.type !== "url") {
      ...
      return {
        ...
        note: "This input is being treated as message/QR/payload content, so QANTRAE shows payload structure instead of pretending it is a normal web link."
      };
    }
    let url = null;
  ...
  function finalize(ctx){
    ...
    // warnings. For the QANTRAE ecosystem UI, we convert that into Risk Score:
    ...
    let summary = "No major local red flag was found in the visible structure. QANTRAE keeps this low, but still recommends normal caution for logins, payments, downloads, or private data.";
    ...
    else if (score >= 20 || high || medium >= 2) { risk = "Needs Review"; riskClass = "risk-medium"; next = "Verify first"; summary = "This target needs a second look. QANTRAE found caution evidence, so verify the sender, root domain, and reason for the link before acting."; }
  ...
  function makePlainLanguageVerdict(risk, target, dominant, confidence){
    if (target.type === "payload") {
      return "This is not being judged as a normal website. QANTRAE first classifies what the QR/text payload appears to be, then tells the user whether to identify it, verify it, or stop.";
    }
    ...
    return "The visible structure looks locally clean. QANTRAE did not find a major offline warning pattern, but the user should still verify context before logging in, paying, or sharing private information.";
  }
  ...
  function makeOrgEscalation(risk, target, dominant, confidence){
    ...
    if (risk === "High Risk") return "Escalate as a security case. Preserve the raw URL, source message, timestamp, screenshot if available, and QANTRAE evidence list.";
  ...
  function makeHumanWhy(risk, target, dominant, profile){
    if (target && target.type === "payload") {
      return risk === "Low Risk"
        ? "QANTRAE treated the input as a payload instead of forcing it into website scoring. Context still matters before using codes or QR data."
  ...
  function makeMemorySummary(){
    ...
    return [
      "QANTRAE IT Local Memory Summary",
  ...
  function renderPatternMemoryDashboard(){
    ...
    if (els.memoryDashboardNote) {
      els.memoryDashboardNote.textContent = (memory.total || 0)
        ...
        : "Run a few scans to let QANTRAE build local pattern awareness in this browser.";
    }
  }
  ...
  const ONLINE_INTEL_PROVIDER_MANIFEST = Object.freeze({
    ...
    hardRules: Object.freeze([
      ...
      "If the provider is unavailable, QANTRAE must fall back to the local report.",
  ...
  function makeOnlineIntelRequestPlan(report){
    ...
    return {
      ...
      fallback:"Use the existing offline QANTRAE report if the user declines, the provider is unavailable, or the network is offline."
    };
  }
  ...
  function revokeOnlineConsent(){
    ...
    renderOnlineConsentPanel(lastReport || emptyReport());
    if (els.copyStatus) els.copyStatus.textContent = "Online Intel consent cleared. QANTRAE is locked to offline-first mode.";
  }
  ...
  function enableOnlineConsent(){
    const state = {
      ...
      requiresFreshConsent:true,
      note:"v3.29.3 allows a user-clicked RDAP bridge lookup only after consent is armed, with host-aware fallback messages when the bridge is not available. QANTRAE still performs no hidden background lookup."
    };
  ...
  function makeOnlineArchitectureSummary(report){
    ...
    if (consent.status === "armed") {
      return "Online Intel gate is armed. QANTRAE can generate user-opened lookup links for " + root + ", but it still makes no hidden fetch/API/background request.";
    }
    return "Online Intel is locked off. QANTRAE will stay offline unless the user deliberately arms the explicit choice and opens an external lookup link.";
  }
  ...
  function renderOnlineConsentPanel(report){
    ...
    if (els.onlineConsentDetail) {
      els.onlineConsentDetail.textContent = armed
        ? "Online Intel launchpad is armed. Generate lookup links only when you are ready to open an external site. QANTRAE does not run hidden online checks."
  ...
  function generateOnlineLookupLinks(){
    ...
    if (consent.status !== "armed") {
      ...
      if (els.copyStatus) els.copyStatus.textContent = "Arm Online Intel consent first. QANTRAE did not open or send anything online.";
      return;
    }
    renderOnlineLookupLaunchpad(lastReport || emptyReport());
    if (els.onlineArchitectureNote) els.onlineArchitectureNote.textContent = "Lookup links generated locally. Nothing opens until you click a link; each link leaves QANTRAE and opens an external site.";
    if (els.copyStatus) els.copyStatus.textContent = "Online lookup links generated. QANTRAE did not run hidden fetch/API/background requests.";
  }
  ...
  function getRdapHostContext(){
    ...
    const isVercel = host.endsWith("vercel.app") || !!(window.QANTRAE_RDAP_BRIDGE_BASE);
  ...
  function makeLocalRdapEndpoint(domain){
    const overrideBase = window.QANTRAE_RDAP_BRIDGE_BASE ? String(window.QANTRAE_RDAP_BRIDGE_BASE).replace(/\/$/, "") : "";
  ...
  async function runConsentGatedRdapLookup(){
    ...
    if (hostContext.isFile || hostContext.isGithubPages) {
      ...
      if (els.copyStatus) els.copyStatus.textContent = hostContext.detail + " No online lookup was sent from QANTRAE.";
      return;
    }
  ...
  function applyLocalPatternMemory(report){
    ...
    if (notes.length) {
      ...
      report.summary = (report.summary || "") + " Local pattern memory also noticed repetition on this device, so QANTRAE recommends a little extra verification.";
  ...
  function guidanceFor(report){
    if (!report || !report.raw) {
      return {
        ...
        whyText:"QANTRAE will explain the strongest warning sign in plain language after analysis.",
  ...
  function buildUserReportText(report){
    if (!report || !report.raw) return "No QANTRAE IT scan has been run yet.";
    return [
      "QANTRAE IT User View",
  ...
  function buildProfessionalSummaryText(report){
    ...
    return report.caseProfessionalSummary || [
      "QANTRAE IT reviewed this target locally.",
  ...
  function buildExportEnvelope(report, packetCore){
    ...
    return {
      ...
      generatedBy: "QANTRAE IT local browser engine",
  ...
  function buildCasePacketJson(report){
    ...
    const packet = {
      ...
      verdict: { risk: report.risk, riskScore: report.score, nextMove: report.next, confidence: report.shellConfidence || { level: report.confidence || report.confidenceBrief || 'Not stated' }, primaryTrigger: report.primaryTrigger, threatLaneId: report.threatLaneId || "LANE-NONE-000", threatLane: report.threatLaneReportName || report.threatLaneLabel || "No dominant lane" },
      evidenceProvenance: report.evidenceProvenance || { verdictBasis: ['Offline local analysis'], supplementalSources: [], sourcesChecked: ['Offline'], note: 'No live evidence was attached to this export.' },
      ...
      onlineIntelFindings: { source: "user-entered notes from consent-opened external lookups", notes: report.onlineFindingsNotes || currentOnlineFindingsNotes() || "", privacy: "Stored locally only; QANTRAE does not verify notes automatically." },
      ...
      privacy: "The verdict and risk score were generated from local visible-input analysis. Any attached RDAP, VirusTotal, Google Web Risk, or redirect-resolution evidence is supplemental and does not silently rewrite the offline verdict."
    };
  ...
  function buildCleanSummaryText(report){
    if (!report || !report.raw) return "No QANTRAE IT scan has been run yet.";
    return (report.reportOneLine || ("QANTRAE IT marked this as " + report.risk + " with a risk score of " + report.score + "/100.")) + " " +
      "Main reason: " + ((report.primaryReason && report.primaryReason[0]) || report.primaryTrigger || report.explain || "No single trigger") + ". " +
      "What to do next: " + (report.reportWhatToDoNext || report.next) + ". " +
  ...
  function buildCasePacketText(report){
    ...
    return [
      ...
      "Interpretation Confidence: " + ((report.shellConfidence && report.shellConfidence.level) || report.confidence || report.confidenceBrief || "Not stated"),
      "Verdict Basis: Offline local analysis",
      "Supplemental Sources: " + (((report.evidenceProvenance && report.evidenceProvenance.supplementalSources) || []).join(", ") || "None run"),
      "Provenance Note: " + ((report.evidenceProvenance && report.evidenceProvenance.note) || "Live evidence is supplemental and does not silently change the offline score."),
      "",
      ...
      "The verdict and risk score were generated locally in the browser. If supplemental live evidence is listed above, it was added only after an explicit user action and did not silently change the offline score.",
      "",
      "JSON NOTE",
      "Use Download JSON for structured evidence import or future QANTRAE tools. Use this TXT export for readable sharing and record keeping."
    ].join("\n");
  }
  ...
  function explainVerdict(){
    if (!lastReport || !lastReport.raw) { if (els.copyStatus) els.copyStatus.textContent = "Run a scan first, then QANTRAE can explain the verdict in plain language."; return; }
  ...
  function stopQrScanner(){
    ...
    if (qrActiveVideo) { try { qrActiveVideo.pause(); } catch(_){} try { qrActiveVideo.srcObject = null; } catch(_){} }
    if (els.qrVideo && els.qrVideo !== qrActiveVideo) { try { els.qrVideo.pause(); } catch(_){} try { els.qrVideo.srcObject = null; } catch(_){} }
    qrActiveVideo = null;
  ...
  function submitQrValue(value){
    ...
    stopQrScanner();
    window.QANTRAEPendingScanSource = "qr-scanner";
    runScan("qr-scanner");
    return true;
  }
  ...
  function isLocalQrDecoderReady(){
    return !!(window.jsQR && window.QANTRAELocalQrDecoder && window.QANTRAELocalQrDecoder.available === true);
  }
  ...
  async function scanQrFrame(){
    const video = qrActiveVideo || els.qrVideo;
    if (!qrScanLoopActive || !video) return;
    try {
  ...
  async function startQrScanner(targetVideo){
    const videoTarget = targetVideo || els.qrVideo;
    if (!videoTarget) throw new Error("Camera preview element is unavailable");
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      const msg = window.isSecureContext ? "Camera access is unavailable in this browser." : "Camera requires HTTPS or localhost in this browser.";
      setQrStatus(msg + " Paste decoded QR text manually.");
      throw new Error(msg);
    }
    try {
      if (els.qrStartButton) els.qrStartButton.disabled = true;
      qrDetector = null;
      qrDecoderMode = "native";
      if ("BarcodeDetector" in window) {
        setQrStatus("Requesting camera permission. QANTRAE stays local and uses the browser QR detector when available.");
        qrDetector = new BarcodeDetector({ formats: ["qr_code"] });
      } else {
        qrDecoderMode = "jsqr";
        setQrStatus("This browser lacks native QR detection. QANTRAE is loading the packaged local jsQR decoder. Camera processing stays on this device.");
      ...
      qrActiveVideo = videoTarget;
      videoTarget.muted = true;
      videoTarget.autoplay = true;
      videoTarget.playsInline = true;
      videoTarget.setAttribute("muted", "");
      videoTarget.setAttribute("autoplay", "");
      videoTarget.setAttribute("playsinline", "");
      videoTarget.srcObject = qrStream;
      if (!targetVideo && els.qrCameraPanel) els.qrCameraPanel.hidden = false;
      if (els.qrStopButton) els.qrStopButton.hidden = false;
      if (videoTarget.readyState < 1) {
        await new Promise((resolve,reject)=>{
          const timer=setTimeout(()=>reject(new Error("Camera stream opened but video metadata did not load")),6000);
          const done=()=>{clearTimeout(timer);resolve();};
          videoTarget.addEventListener("loadedmetadata",done,{once:true});
        });
      }
      await videoTarget.play();
      qrScanLoopActive = true;
      setQrStatus(qrDecoderMode === "jsqr" ? "Camera active with local compatibility decoder. Center a QR code in the frame; QANTRAE will scan locally in the browser." : "Camera active locally. Center a QR code in the frame; QANTRAE will scan the decoded payload offline.");
  ...
  function runScan(source){
    const scanSource = (typeof source === "string" && source) || window.QANTRAEPendingScanSource || "analyze-button";
    window.QANTRAEPendingScanSource = "";
    ...
    window.QANTRAELegacyLastReport = lastReport;
    const activeFoundationState = window.QANTRAEScanState?.getState?.();
    ...
    if (typeof window.QANTRAEApplyLegacyScanReport === "function") {
      try { window.QANTRAEApplyLegacyScanReport(lastReport, scanSource); } catch (error) { console.error("Dashboard sync hook failed", error); }
    }
  }

  window.QANTRAERunLocalScan = runScan;
  window.QANTRAEGetLastReport = () => lastReport;
  window.QANTRAEStartQrScanner = startQrScanner;
  window.QANTRAEStopQrScanner = stopQrScanner;
  window.QANTRAESaveCurrentCase = saveCase;
  window.QANTRAEGetSavedCases = () => readHistory();
  window.QANTRAEDownloadCaseText = downloadCaseText;
  window.QANTRAEDownloadCasePacket = downloadCasePacket;
  window.QANTRAEBuildCasePacketJson = report => buildCasePacketJson(report || lastReport);
  ...
  function buildHistoryText(item){
    ...
    return [
      "QANTRAE IT LOCAL SCAN HISTORY ITEM",
  ...
  document.querySelectorAll("[data-example]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      window.QANTRAEPendingScanSource = "example-button";
      els.scanButton.click();
    });
  });
  els.scanButton.addEventListener("click", () => runScan(window.QANTRAEPendingScanSource || "analyze-button"));
  if (els.input) els.input.addEventListener("input", updateDetectedInputType);
  updateDetectedInputType();
  els.input.addEventListener("keydown", event => {
    if (event.key === "Enter") {
      event.preventDefault();
      window.QANTRAEPendingScanSource = "keyboard-enter";
      els.scanButton.click();
    }
  });
  ...
  if (els.clearAllLocalDataButton) els.clearAllLocalDataButton.addEventListener("click", () => {
    ...
    renderPatternMemoryDashboard();
    renderOnlineConsentPanel(lastReport || emptyReport());
    if (els.copyStatus) els.copyStatus.textContent = "All QANTRAE IT local scan/history/memory choices were cleared on this device.";
  });
  ...
  function setupIntelligenceDrawer(){
    ...
    activate("timelinePanel", { reveal: false });
  }
